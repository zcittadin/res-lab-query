...
public class ParcialReportTemplate {
	...
	public static ComponentBuilder<?, ?> createHeaderComponent(Produto produto) {
		return cmp.horizontalList().add(cmp.horizontalList(
				...
				cmp.verticalList(
						cmp.text("Relatório de processo (parcial)").setStyle(bold18CenteredStyle)
	...
	public static ComponentBuilder<?, ?> createChartComponent(Produto produto, LineChartConfig lineChartConfig) {
		return cmp.horizontalList(createLineChart(produto, lineChartConfig));
	}
	...
	public static ComponentBuilder<?, ?> createDadosComponent(Produto produto, String periodo) {
		return cmp.horizontalList(
				cmp.verticalList(cmp.text("Data de produção: " + dataSdf.format(produto.getDtInicial())),
					cmp.text("Tempo decorrido: " + periodo),
	...
	private static TimeSeriesChartBuilder createLineChart(Produto produto, LineChartConfig lineChartConfig) {
		...
		if (!lineChartConfig.getReatorA()) {
			return cht.timeSeriesChart().setTitle("Temperatura x tempo").setTitleFont(boldFont)
					.setShowShapes(false).seriesColors(Color.ORANGE).setShowLegend(Boolean.TRUE)
					.setTimePeriod(horarioColumn).setTimePeriodType(TimePeriod.SECOND)
					.series(cht.serie(tempCaldeiraColumn)).setTimeAxisFormat(cht.axisFormat()).setValueAxisFormat(cht
							.axisFormat().setLabel("ºC").setRangeMaxValueExpression(350).setRangeMinValueExpression(0));
		}
		if (!lineChartConfig.getCaldeiraA()) {
			return cht.timeSeriesChart().setTitle("Temperatura x tempo").setTitleFont(boldFont)
					.setShowShapes(false).seriesColors(Color.RED).setShowLegend(Boolean.TRUE)
					.setTimePeriod(horarioColumn).setTimePeriodType(TimePeriod.SECOND)
					.series(cht.serie(tempReatorColumn)).setTimeAxisFormat(cht.axisFormat()).setValueAxisFormat(cht
							.axisFormat().setLabel("ºC").setRangeMaxValueExpression(350).setRangeMinValueExpression(0));
		}
		return cht.timeSeriesChart().setTitle("Temperatura x tempo").setTitleFont(boldFont).setShowShapes(false)
				.seriesColors(Color.RED, Color.ORANGE).setShowLegend(Boolean.TRUE).setTimePeriod(horarioColumn)
				.setTimePeriodType(TimePeriod.SECOND).series(cht.serie(tempReatorColumn), cht.serie(tempCaldeiraColumn))
